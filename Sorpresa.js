...
// SORPRESA.JS
// Secuencia:
// 🕯️ Vela → 🎉 Confeti → ❤️ Corazones → 🎈 Globos
// → 🎁 Regalo → 💌 Carta → 🌙 Escena final
// ======================================================
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ======================================================
  // POSICIONAR LLAMA EXACTAMENTE SOBRE LA VELA
  // ======================================================

  function posicionarLlama() {

    if (!llama || !vela || velaApagada) return;

    const rectVela = vela.getBoundingClientRect();

    const centroX =
      rectVela.left + rectVela.width / 2;

    const topLlama =
      rectVela.top - 30;

    llama.style.position = "fixed";
    llama.style.left = `${centroX - 10}px`;
    llama.style.top = `${topLlama}px`;
    llama.style.bottom = "auto";
    llama.style.zIndex = "10001";
  }


  requestAnimationFrame(() => {
    requestAnimationFrame(posicionarLlama);
  });
